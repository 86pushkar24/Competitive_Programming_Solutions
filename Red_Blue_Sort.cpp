...
void push() {
    int n;ci n;
    // vi vec(n+1),mnPre(n+2),mxPre(n+2),smPre(n+2),mnSf(n+2),mxSf(n+2),smSf(n+2);
    // int sm(0);
    // for1(i,n){
    //     ci vec[i];
    //     sm+=vec[i];
    // }
    // mnPre[0]=n+1,mxPre[0]=0,smPre[0]=0;
    // for1(i,n){
    //     mnPre[i]=min(mnPre[i-1],vec[i]);
    //     mxPre[i]=max(mxPre[i-1],vec[i]);
    //     smPre[i]=smPre[i-1]+vec[i];
    // }
    // mnSf[n+1]=n+1,mxSf[n+1]=0,smSf[n+1]=0;
    // rfl(n,1){
    //     mnSf[i]=min(mnSf[i+1],vec[i]);
    //     mxSf[i]=max(mxSf[i+1],vec[i]);
    //     smSf[i]=smSf[i+1]+vec[i];
    // }
    // int fp(0);
    // for1(i,n){
    //     if(vec[i]==i)fp++;
    ...
    // int ans(fp);
    // for1(i, n - 1) {
    //     int mnP(mnPre[i]),mxP(mxPre[i]),smP(smPre[i]);
    //     int exp=i*(i+1)/2;
    //     int mnS=mnSf[i+1],mxS=mxSf[i+1],smS=smSf[i+1];
    //     int exp2=(n-i)*(n+i+1)/2;
    //     if((mnP==1&&mxP==i&&smP==exp) && (mnS==i+1&&mxS==n&&smS==exp2)) {
    //         int pf(n-1);
    //         ans=max(ans,pf);
    //     }
    // }
    ...
    if(vec[0]==1||vec[n-1]==n||res>0)res=max(res, n - 1);
    else res=max(res,n-2);
    cou(res);
}
