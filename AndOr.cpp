...
// Input/Output Macros
#define cin(a) int n;cin >> n;vi a(n+1);for1(i, n){ cin >> a[i]; }
#define co(a){cout << a << ' ';}
#define cou(a){cout << a << "\n";}
#define ci cin >>
#define sz(c) c.size()
...
void push(){
    ...
    fl(2,n+1){
        ...
        else if(v0[i]==1) st2.ins(i);
    }
    auto ans=[&]()->int{
        if(n==1) return v0[1];
        int res=-1;
        if(!st1.empty()) res=max(res,*st1.rbegin());
        if(!st2.empty()) res=max(res,*st2.rbegin());
        if(res==-1) return v0[1];
        if(res%2!=1) return 0;
        return 1;
    };
    while(q--){
