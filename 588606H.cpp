...
#define vpii vector<pair<int, double>>
...
double fn(int x,int y,int t1,int t2,double nw){
    double h1=x*t1*nw+y*t2;
    double h2=y*t1*nw+x*t2;
    return max(h1,h2);
}
bool chk(pii &p1,pii &p2){
    return (fabs(p1.se-p2.se)<1e-9)?(p1.fi<p2.fi):(p1.se>p2.se);
}
void push(){
    int n,t1,t2,k;
    ci n>>t1>>t2>>k;
    double nw=(100.0-k)/100.0;
    vpii vp(n);
    for0(i,n){
        int x,y;
        ci x>>y;
        vp[i]={i+1,fn(x,y,t1,t2,nw)};
    }
    sort(aint(vp),chk);
    for(auto &[it,ht]:vp)cout<<it<<" "<<fixed<<setprecision(2)<<ht<<endl;
}
