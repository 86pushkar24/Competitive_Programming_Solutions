...
void push(){
    string s;
    cin >> s;
    int n = s.size(), b(0), aCnt(0), aTot(0);
    vi a;
    for0(i,n)
    {
        if(s[i] == 'a') aCnt++, aTot++;
        else if(s[i] == 'b') if(aCnt) a.pb(aCnt), aCnt = 0; b = 1;
    }
    if(b && aCnt) a.pb(aCnt);
    if(!a.size()) {
        cou(aTot)
    ...
    int res(1);
    for0(i,a.size()) res = (res * (a[i] + 1)) % MOD;
    cou(res - 1)
}
...
signed main()
{
    ...
    // cerr << "Time measured: " << elapsed.count() * 1e-9 << " seconds." << nl;
}
