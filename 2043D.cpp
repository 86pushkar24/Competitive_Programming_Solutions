...
#define sumv(arr) accumulate(aint(arr), 0int)
...
void push()
{
    ...
    auto upd = [&](int p, int q){
        if (!chk(p * g, q * g))return;
        int a = p * g;
        int b = q * g;
        if (abs(A - B) < abs(a - b))A = a, B = b;
        if (abs(A - B) == abs(a - b) && A > a)A = a, B = b;
    };
    for (int p = s; p <= min(e, s + lim); ++p)for (int q = e; q >= max(s, e - lim); --q)
            if (__gcd(p, q) == 1)
            {
                upd(p, q);
                break; // vld pr fnd!!
