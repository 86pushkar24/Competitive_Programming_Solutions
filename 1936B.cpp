...
void push(){
    ...
    ci s;
    vi v0,v1;
    int p0[n+2],p1[n+2],c0[n+2],c1[n+2];
    p0[0]=p1[0]=c0[0]=c1[0]=0;
    for0(i,n)if(s[i]=='>')v0.pb(i+1);
        else v1.pb(i+1);
    for1(i,n){
        p0[i]=p0[i-1];
        p1[i]=p1[i-1];
    ...
    for1(i,n){
        ...
        if(s[i-1]=='>'){
            ...
            int y=c1[n]-c1[i];
            if(c0[i]<=y){
                x=p1[v1[c0[i]-1+c1[i]]]-p1[i];
                y=c0[i];
                ans=2*(x-i*y)+2*(i*c0[i]-p0[i])+i;
            }
            else{
                int z=p0[i-1]-p0[v0[c0[i]-y-1]-1];
                int w=y;
                ans=2*(x-i*y)+2*(i*w-z)+(n-i+1);
            }
        }
        else{
            int x=p1[n]-p1[i-1];
