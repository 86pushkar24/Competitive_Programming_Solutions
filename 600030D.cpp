...
void push(){
    ...
    for1(i,n)ci v1[i];
    int rt=-1;
    for1(i,n)if(v0[i]==-1)rt=i;
    vvi bache(n+1);
    for1(i,n)if(v0[i]!=-1)bache[v0[i]].pb(i);
    vi v(n+1,-1);
    que q;
    v[rt]=0;
    q.push(rt);
    while(!q.empty()){
        int node=q.front();
        q.pop();
        for(int it:bache[node]){
            v[it]=v[node]+1;
            q.push(it);
        }
    }
    vvi dada(20,vi(n+1,-1));
    for1(i,n)if(v0[i]!=-1)dada[0][i]=v0[i];
    for1(k,19)for1(i,n)if(dada[k-1][i]!=-1)dada[k][i]=dada[k-1][dada[k-1][i]];
    int crr=x;
    bool ans=0;
    for1(i,k){
        int m=k-i+1;
        if(v[crr]<m){
            ...
            int y=crr;
            for0(bit,20)if(m&(1<<bit))y=dada[bit][y];
            crr=v1[y];
        }
    }
    cy(ans);
