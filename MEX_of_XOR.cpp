...
void push()
{
    ...
    for (int i = 29; i >= 0; --i)
    {
        unsigned int l_i = (l >> i) & 1;
        unsigned int r_i = (r >> i) & 1;
        unsigned int x_i = (x >> i) & 1;
        bool can_zero = false, can_one = false;
        unsigned int y_i_zero = x_i;
        unsigned int y_i_one = 1 ^ x_i;
        if (l_i == r_i)
        {
            unsigned int allowed_y_i = l_i;
            if (y_i_zero == allowed_y_i)
                can_zero = true;
            if (y_i_one == allowed_y_i)
                can_one = true;
        }
        else
        {
