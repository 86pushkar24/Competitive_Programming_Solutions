...
void push(){
    ...
    for(int i=2;i<=n;i++)if(p1[i]==0)for(int j=i;j<=n;j+=i)p1[j]++;
    for1(i,n){
        for(int j=i;j<=n;j+=i){
            if(p1[i]==1)p3[j]++;
        }
    ...
    for0(i,n){
        ...
        if(p3[a]==1)m1[a]++,s++;
            ...
            m2[a]++;
            m3[p2[a]]++;
            if(a/p2[a]!=p2[a])m3[a/p2[a]]++;
        }
    }
    int ans=0;
    for(auto [x,y]:m1)ans+=(s-y)*y;
    ans/=2;
    for(auto [x,y]:m1)ans+=(int)m3[x]*y;
    for(auto [x,y]:m2)ans+=(int)y*(y+1)/2;
    cou(ans);
}
