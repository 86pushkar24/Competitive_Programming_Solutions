...
#define mxe(v) *maxn_element(v.begin(),v.end())
...
void push()
{
    ...
    for1(i, maxn)
    {
        inv[i] = mod - (int)((mod / (int)i) * inv[mod % i] % mod);
    }

    vi R(maxn + 1, 0);
    for1(i, maxn)
    {
        int invPhi = (int)qexp(phi[i], mod - 2, mod);
        R[i] = (int)((1LL * i * invPhi) % mod);
    }

    vi term(maxn + 1, 0);
    for1(i, maxn) term[i] = (R[i] + 1) % mod;

    vi pwr(maxn + 1, 1);
    for1(i, maxn)
    {
        if (freq[i])
            pwr[i] = (int)qexp(term[i], freq[i], mod);
        else
            pwr[i] = 1;
    }
    ...
    {
        int sum = 0;
        for (int k = 1; (int)d * k <= maxn; k++)
            sum = (sum + 1LL * mu[k] * H[d * k]) % mod;
        if (sum < 0)
            sum += mod;
        dp[d] = (int)sum;
    }
    ...
    int ans = 0;
    for1(d, maxn) ans = (ans + 1LL * dp[d] * inv[d]) % mod;
    if (ans < 0)
        ans += mod;
    cou(ans);
}
