...
void push(){
    int n,m;
    ci n>>m;
    vvi v(n,vi(m)),v1(n,vi(m));
    for0(i,n)for0(j,m)ci v[i][j];
    for0(i,n)for0(j,m)ci v1[i][j];
    if(n>1 && m>1){
        int ct1(0),ct2(0);
        for0(i,n)for0(j,m)ct1^=v[i][j];
        for0(i,n)for0(j,m)ct2^=v1[i][j];
        cy(ct1==ct2);
        return;
    }
    int k=n*m;
    ...
    t1.reserve(k);
    t2.reserve(k);
    for0(i,n)for0(j,m)t1.pb(v[i][j]);
    for0(i,n)for0(j,m)t2.pb(v1[i][j]);
    int c1(1),c0(1),c11(1),c01(1);
    for(auto it:t1){
        if(it==0)c0=0;
        if(it==1)c1=0;
    }
    ...
    int ans1,ans2;
    if(c1)ans1=0;
    else if(c0)ans1=(k%2)+1;
    else ans1=2;
    if(c11)ans2=0;
    else if(c01)ans2=(k%2)+1;
    else ans2=2;
    cy(ans1==ans2);
}
