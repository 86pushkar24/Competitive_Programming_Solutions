...
const int arr[7]={1,10,100,1000,10000,100000,1000000};
void push(){
    int x,y,z;
    int k;
    ci x>>y>>z>>k;
    if(z>max(x,y)+1||z<max(x,y)){
        cou(-1);
        return;
    }
    int a(-1),b(-1),c(-1);
    fl(arr[x-1],arr[x]){
        int minB(inf);
        if(i+arr[y-1]>=arr[z-1]&&i+arr[y-1]<arr[z])minB=arr[y-1];
        else if(arr[z-1]-i>=arr[y-1]&&arr[z-1]-i<arr[y])minB=arr[z-1]-i;
        int maxB(-inf);
        if(i+arr[y]-1>=arr[z-1]&&i+arr[y]-1<arr[z])maxB=arr[y]-1;
        else if(arr[z]-1-i>=arr[y-1]&&arr[z]-1-i<arr[y])maxB=arr[z]-1-i;
        if(minB<=maxB){
            int cnt=maxB-minB+1;
            if(cnt>=k){
                a=i,b=minB+k-1,c=a+b;
                k=0;
                break;
            }
            else k-=cnt;
