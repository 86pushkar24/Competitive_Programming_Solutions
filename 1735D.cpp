...
#define fl(a,b)for(int j=a;j<(b);++j)
...
void push(){
    ...
    for0(i, n){
        fl(i + 1, n){
            string req;
            for0(l, k){
                if (input[i][l] == input[j][l]) req += input[i][l];
                else req += (char)('0' + (3 - (input[i][l] - '0') - (input[j][l] - '0')));
            }
            if (!mp.count(req) || mp[req] <= j)continue;
            v[i]++, v[j]++, v[mp[req]]++;
        }
    }
    int ans(0);
    for0(i, n)if (v[i] >= 2)ans += (v[i] * (v[i] - 1)) / 2;
    cou(ans);
}
