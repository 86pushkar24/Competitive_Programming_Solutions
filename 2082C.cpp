...
int inv(int a){return qexp(a, mod-2, mod);}
string add(string s){
    int k=1;
    rfl(s.size()-1, 0){
        if(s[i]=='0'){
...
pii fn(const string &s, map<string, pii> &mm){
    if(s=="1")return {0, 0};
    if(mm.count(s))return mm[s];
    pii res;
    if(s.back()=='0'){
        auto [p, k]=fn(s.substr(0, s.size()-1), mm);
        res={(qexp(2, k, mod)+p)%mod, k};
    }
    else{
        string a=s.substr(0, s.size()-1), b=add(a);
        auto [pa, ka]=fn(a, mm);
        auto [pb, kb]=fn(b, mm);
        int pka=qexp(2, ka, mod),pkb=qexp(2, kb, mod);
        res.fi=(qexp(2, (ka+kb+1), mod)+(pa*pkb%mod)+(pb*pka%mod))%mod;
        res.se=ka+kb+1;
    }
    return mm[s]=res;
}
