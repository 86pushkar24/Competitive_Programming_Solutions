...
void push(){
    ...
    for0(i,mnn){
        ...
        if(l>rt||tp>bt)break;
        vector<char> v2;
        for(int j=l;j<=rt;j++) v2.push_back(v[tp][j]);
        for(int j=tp+1;j<=bt;j++)v2.push_back(v[j][rt]);
        if(bt>tp) for(int j=rt-1;j>=l;j--)v2.push_back(v[bt][j]);
        if(rt>l) for(int j=bt-1;j>tp;j--)v2.push_back(v[j][l]);
        if (v2.size() < 4)continue;
        int ct(0);
        string str(v2.begin(), v2.end());
        str += str.substr(0, 3);
        for0(i,v2.size()) if(str.substr(i,4)=="1543")ct++;
        res += ct;
    }
    cou(res)
}
