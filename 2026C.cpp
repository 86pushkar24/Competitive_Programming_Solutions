...
#define fi first
#define se second
#define cin(a) int n; cin>>n; vi a(n); for (int i = 0; i < n; i++) {cin>>v.pb(i);}
#define co(a) {cout<<a<<' ';}
#define cou(a) {cout<<a<<"\n";}
...
void push(){
    ...
    vi vis(n);
    set<int> nex;
    for0(i,n) nex.insert(i+1);
    set<int> used;
    for(int i=n;i>=1;i--)
    {
        if(s[i-1]=='0')
        {
            if(nex.find(i)==nex.end()) continue;
            while(sz(used))
            {
                int k=*used.begin();
                k*=-1;
        ...
        {
            if(sz(nex)==0 or nex.find(i)==nex.end())
            {
                continue;
            }
            int k=*nex.begin();
