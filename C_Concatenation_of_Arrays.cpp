...
bool comp(const vi&n1,const vi&n2){
    int mp1=max(n1[0],n1[1]);  
    int kp1=min(n1[0],n1[1]);  
    int mp2=max(n2[0],n2[1]);  
    int kp2=min(n2[0],n2[1]);  
    if(mp1==mp2)return kp1<kp2;
    return mp1<mp2;
}
void push(){
	int n;cin>>n;
    vector<vi>nums(n,vi(2));
    for0(i,n)cin>>nums[i][0]>>nums[i][1];
    sort(nums.begin(),nums.end(),comp);
    for0(i,n)cout<<nums[i][0]<<' '<<nums[i][1]<<' ';
    cout<<endl;
}
