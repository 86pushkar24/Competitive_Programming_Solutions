...
pii push(int n,int k){
    if(n<=k)return{-1,-1}; 
    // if(n>=2*k+1)return{n,1};
    // int b=n-k;
    // if(b>=1){
    //     int d=gcd(n,b);
    //     int lcm=(n/d)*b; 
    //     if(lcm-d>=2*k)return{n,b};
    // }
    // rfl(n-k,1){
    //     if(gcd(n,i)==1){
    //         if(n*i-1>=2*k)return{n,i};
    //         break;
    //     }
    // }
    // return{-1,-1};
    ...
    for(int b=nw;b>=1;b--){
        int d=gcd(a,b);
        int lcm=(a/d)*b; 
        if(lcm-d>=2*k&&abs(a-b)>=k)return{a,b};
        if(a-b<k)break;
    }
    if(n>=k+2){
        a=n-1;
