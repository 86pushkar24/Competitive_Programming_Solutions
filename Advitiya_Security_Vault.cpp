...
int fn3(int a,int k){return (a==0?k:1);}
int fn4(int a,int k){return (a==0?k:1);}
...
void push(){
    ...
    int k0(pre(k%mod,cnt_0));
    int cst(1);
    for0(i,n/2){
        int x(v0[i]),y(v0[n-1-i]);
        int z(fn1(x,y,k%mod));
        cst=(cst*z)%mod;
    }
    if(n%2==1) cst =(cst*fn3(v0[n/2],k%mod))%mod;
    int k1(cst);
    int cstp(1);
    for0(i,n/2){
        int x(v0[i]),y(v0[n-1-i]);
    ...
    if(n%2!=0) cstp=(cstp*fn4(v0[n/2],k%mod))%mod;
    int k2(cstp);
    int rem(k1-k2);
    if(rem<0)rem+=mod;
    int lft((rem*modin)%mod);
    int res(k0-lft);
    if(res<0)res+=mod;
    cou(res%mod);
}
