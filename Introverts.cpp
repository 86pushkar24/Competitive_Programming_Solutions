...
void push(){
    ...
    mts mt;
    seg.ins({0,n+1});
    auto gt=[&](int lt,int rt,int n)->int {
        if(lt==0 && rt==n+1) return n;
        if(lt==0) return rt-1;
        if(rt==n+1) return n-lt;
        return (rt-lt)/2;
    };
    mt.ins(gt(0,n+1,n));
    for1(i,n){
        ...
        vi v1=fn(lt,rt,n);
        if(find(aint(v1),k)==v1.nd){
            ans=0;
            break;
        }
        seg.erase(a1);
        auto a2=mt.find(t);
        if(a2!=mt.nd) mt.erase(a2);
        if(k-lt>1){
            seg.ins({lt,k});
            mt.ins(gt(lt,k,n));
        }
