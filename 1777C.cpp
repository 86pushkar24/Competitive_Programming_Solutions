    ...
    while (r < n){
        while (r < n && count < m){
            for1(j, sqrt(v[r])){
                if (v[r] % j == 0){
                    freq[j]++;
                    if (freq[j] == 1 && j <= m)count++;
                    int div(v[r] / j);
                    if (div != j){
                        freq[div]++;
                        if (freq[div] == 1 && div <= m)count++;
                    }
                }
            }
            r++;
        }
        if (count >= m)ans = min(ans, v[r - 1] - v[l]);
        while (l < r && count >= m){
            for1(j, sqrt(v[l])){
                if (v[l] % j == 0){
                    freq[j]--;
                    if (freq[j] == 0 && j <= m)count--;
                    int div(v[l] / j);
                    if (div != j){
                        freq[div]--;
                        if (freq[div] == 0 && div <= m)count--;
                    }
                }
            }
            l++;
