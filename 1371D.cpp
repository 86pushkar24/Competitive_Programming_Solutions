...
void push()
{
    ...
    int maxrow(0), minrow(INT_MAX);
    int maxcol(0), mincol(INT_MAX);
    ...
    for0(i, n)
    {
        ...
        maxrow = max(cntrow, maxrow);
        minrow = min(cntrow, minrow);
        maxcol = max(cntcol, maxcol);
        mincol = min(cntcol, mincol);
    }

    cou((maxrow - minrow) * (maxrow - minrow) + (maxcol - mincol) * (maxcol - mincol));
