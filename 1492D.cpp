...
void push()
{
    ...
    // Case 3: Impossible to satisfy `k`
    if (k > a + b - 2)
    {
        no
        return;
    ...
    // Case 4: Construct valid strings
    string s(a + b, '0'), t(a + b, '0');
    s[0] = t[0] = '1'; 
    s[1] = '1';        
    t[1 + k] = '1';    

    // Place remaining `1`s to balance `b - 2` remaining `1`s
    for (int i = 0, j = b - 2; i < a + b && j > 0; i++)
    {
        if (s[i] == '0' && t[i] == '0')
        {
            s[i] = t[i] = '1';
