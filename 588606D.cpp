...
void push() {
    ...
    vi v0(n);
    for(auto &x:v0) ci x;
    auto srch=[&](int idx,int x)->bool {
        return binary_search(v0.bg+idx+1,v0.nd,x);
    };
    if(d==1) {
        int kk=0;
        for0(i,n-1) {
    ...
    if(d==2) {
        ...
        for0(i,n) {
            for1(k,59) {
                int rem=(1LL<<k);
                if(v0[i]>v0.back()-rem) break;
                int k1=v0[i]+rem;
                if(!srch(i,k1)) continue;
                int k2=v0[i]+(rem<<1);
                if(srch(i,k2)) {
                    co(v0[i])
                    co(k1)
