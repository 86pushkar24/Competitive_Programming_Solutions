...
void push() {
    ...
    for1(i, 500) {
        for1(j, i - 1) {
            if (__gcd(i, j) > 1) jumps[i].pb(j);
        }
    }
    
    const int MAX_RATING = 4095;
    vvi dp(n + 2, vi(MAX_RATING + 1, 0));
    
    rfl(n - 1, 0) {
        for0(rating, MAX_RATING + 1) {
            if (rating < r[i]) {
                dp[i][rating] = dp[i + 1][rating];
            } else {
                int skip = dp[i + 1][rating];
                int new_rating = rating ^ r[i - 1];
                int max_continue = 0;
                for (int j : jumps[i]) {
                    int next_i = i + j;
                    if (next_i <= n) {
                        max_continue = max(max_continue, dp[next_i][new_rating]);
                    }
                }
                dp[i][rating] = max(skip, new_rating + max_continue);
            }
        }
    }
    
    int jiangly_score = dp[1][3797];
    int tourist_score = dp[1][3892];
    
    if (jiangly_score > tourist_score) {
        cou("JIANGLY");
    } else if (jiangly_score < tourist_score) {
        cou("TOURIST");
    } else {
        cou("DRAW");
    }
