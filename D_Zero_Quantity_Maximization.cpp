...
void push(){
    ...
    for (int a = 0, x, y; a < n; a++)
        if (abs(ar1[a]) > 0 && abs(ar2[a]) > 0)
        {
            bool ok = ar1[a] < 0 || ar2[a] < 0 ? 1 : 0;
            ok = ar1[a] < 0 && ar2[a] < 0 ? 0 : ok;
            x = abs(ar1[a]) / gcd(abs(ar1[a]), abs(ar2[a])), y = abs(ar2[a]) / gcd(abs(ar1[a]), abs(ar2[a]));
            mp1[{x - (ok ? 2 * x : 0), y}]++, mp2[{y - (ok ? 2 * y : 0), x}]++;
        }
        else if (abs(ar1[a]) > 0)
            ct2++;
        else if (ar1[a] == 0 && ar2[a] == 0)
            ct++;
    for (auto i : mp1)
        ans = max(ans, i.se);
    for (auto i : mp2)
