...
void push(){
    int x1,y1,z1,x2,y2,z2,k;ci x1>>y1>>z1>>x2>>y2>>z2>>k;
    int dx(abs(x2-x1)),dy(abs(y2-y1)),dz(abs(z2-z1));
    int a1(max({dx,dy,dz})),a2(dx+dy+dz-a1),a3((a1+k-1)/k),res(0);
    if(a2>=a3-1)res=dx+dy+dz;
    else{
        int ex(a3-a2-1);
        res=dx+dy+dz+((ex>0 && ex%2!=0) ? ex+1 :ex);
    }
    cou(res);
}
