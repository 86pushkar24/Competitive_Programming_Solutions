...
void push(){
    ...
    vi lambai(n+1,1),baap(n+1,1),gherai(n+1,0);
    gherai[0]=-inf;
    auto dfs=[&](auto& self,int u,int p,int d)->void {
        ...
        for(auto v:lt[u]){
            if(v!=p){
                self(self,v,u,d+1);
                baap[v]=u;
                gherai[u]=max(gherai[u],gherai[v]+1);
            }
        }
    };
    dfs(dfs,1,1,0);
    int mx(1);
    for1(i,n)if(lambai[i]>lambai[mx])mx=i;
    vi rasta;
    baap[1]=0;
    while(mx!=0){
        rasta.pb(mx);
    ...
    auto jodo=[&](int u){
        ...
        for(auto v:lt[u]){
            if(v!=baap[u]){
                if(gherai[v]>=gherai[mxx]){
                    sm=mxx;
                    mxx=v;
                }
                else if(gherai[v]>gherai[sm])sm=v;
            }
        }
        if(mxx!=0&&sm!=0)return gherai[mxx]+gherai[sm]+2;
        return gherai[u];
    };
    int res(0);
    for0(i,sz(rasta))res=max(res,jodo(rasta[i])+lambai[rasta[i]]);
    cou(res)
}
