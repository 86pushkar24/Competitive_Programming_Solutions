...
void push(){
    ...
    for0(idx, np)
    {
        ...
        {
            int dist = vp[idx].fi - vp[lst].fi;
            int cst = mn * dist;
            v3[idx] = min(v3[idx], cst);
            debug(v3[idx]);
        }
    }
    int nxt(-1);
    ...
    int ans(0);
    for0(idx, np) if (!chk[idx]) ans += v3[idx];
    for0(i, size(id) - 1)
    {
        int k1 = id[i];
        int k2 = id[i + 1];
        int dist = vp[k2].fi - vp[k1].fi;
        ans += mn * dist;
    }
    cou(ans);
}
