...
void push(){
    ...
    vi v1(n+1,0);
    for1(i,n){
        if(v[i-1]<a)v1[i]=v1[i-1]+1;
        else if(v[i-1]>a)v1[i]=v1[i-1]-1;
        else v1[i]=v1[i-1];
    }
    vi v2(n+1,0);
    for1(i,n){
        if(v[i-1]<b)v2[i]=v2[i-1]+1;
        else if(v[i-1]>b)v2[i]=v2[i-1]-1;
        else v2[i]=v2[i-1];
    }
    int dm(0),dc(0);
    if(b<a)dm=1;
    ...
    for1(i,n+2){
        if(i>=2){
            int k=min(max(0LL,i-2),n);
            int lt=v1[k]+dm;
            for1(q,i-1){
                int m=min(max(0LL,q-1),n),sc=v2[m];
                if(lt>sc)ans++;
            }
        }
        if(i<=n+1){
            int k=min(max(0LL,i-1),n);
            int rt=v1[k];
            for(int q=i+1;q<=n+2;q++){
                int prm=min(max(0LL,q-2),n),sc=v2[prm]+dc;
                if(rt>sc)ans++;
            }
        }
    }
