...
void push(){
    ...
    string s;ci s;
    vi v1;
    for0(i,n)if(s[i]=='?')v1.pb(i);
    int k=sz(v1);
    if(k==0){
        for0(i,n)cout<<(double)(s[i]-'0')<<(i==n-1?'\n':' ');
        return;
    }
    if(n==2&&s=="1?"){
        for0(i,n){
            if(s[i]=='?')cout<<4.5<<(i==n-1?'\n':' ');
            else cout<<(double)(s[i]-'0')<<(i==n-1?'\n':' ');
        }
        return;
    }
    if(n==2&&s=="??"){
        ...
        ans[0] = 5.75;
        ans[1] = 3.25;
        for0(i,2)cout<<ans[i]<<(i==1?'\n':' ');
        return;
    }
    vector<double> ans(n);
    for0(i,n){
        if (s[i] == '?')ans[i] = 4.5;
        else ans[i] = (double)(s[i] - '0');
    }
    for0(i,n)cout<<ans[i]<<(i==n-1?'\n':' ');
}
